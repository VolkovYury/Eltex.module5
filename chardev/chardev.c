// Создает символьное устройство, доступное только для чтения. Возвращает сообщение, с указанием количества произведенных попыток чтения из файла устройства
...
#define SUCCESS 0
#define DEVICE_NAME "chardev"       // Имя устройства, которое будет отображаться в /proc/devices
#define BUF_LEN 80                  // Максимальная длина сообщения


static int Major;                   // Старший номер устройства нашего драйвера
static int Device_Open = 0;         // Устройство открыто? Используется для предотвращения одновременного обращения из нескольких процессов
...
// Вызывается, когда процесс пытается открыть файл устройства, например командой "cat /dev/chardev"
...
static int device_release(struct inode* inode, struct file* file) {
    ...
    // Уменьшить счетчик обращений, иначе, после первой же удачной попытки открыть файл устройства, вы никогда не сможете выгрузить модуль 
...
// Вызывается, когда процесс пытается записать в устройство, например так: echo "hi" > /dev/chardev
static ssize_t
